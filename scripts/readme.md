...
# Updating Icons
The `src/svg` directory is where the icons are downloaded. These are pulled initially from the Figma document, considered the single source of truth. These images are not optimized, as they are just exported from the editor. To get the latest svgs, you will need to run `npx nx ideal-octo-sniffle:update`. Prior to running you will need to configure `figma-icon-config.json`.
...
```sh
FIGMA_ACCESS_TOKEN=[mySampleAccessToken] npx nx run ideal-octo-sniffle:update
```
...
```sh
  npx nx ideal-octo-sniffle:build
```
