...
export const run = async(rootDir: string, optimizeFiles = false) => {
  try {
    ...
    const distIconDir = join(distDir, libraryName);
    ...
    await cleanDirectories(iconDir, distDir, distSvgDir, distIconDir);

    const packageData = await fs.readJson(packageJsonPath);
    const version = packageData.version as string;

    log('Getting SVGs...');
    const srcSvgData = await getSvgs(srcSvgDir, distSvgDir, distIconDir, optimizedOutputSvgDir, optimizeFiles);
...
const cleanDirectories = async (iconDir, distDir, distSvgDir, distIconDir) => {
  await Promise.all([fs.emptyDir(iconDir), fs.emptyDir(distDir)]);
    await fs.emptyDir(distSvgDir), await fs.emptyDir(distIconDir);
}

const getSvgs = async (srcDir: string, distSvgDir: string, distIconDir: string, outputSvgDir: string, isOptimizing: boolean): Promise<SvgData[]> => {
  const optimizedSvgDir = join(distIconDir, 'svg');
