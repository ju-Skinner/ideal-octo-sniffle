...
export const run = async(rootDir: string, optimizeFiles = false) => {
  try {
    ...
    const distIconsDir = join(distDir, libraryName);
    ...
    await cleanDirectories(iconDir, distDir, distSvgDir, distIconsDir);

    const packageData = await fs.readJson(packageJsonPath);
    const version = packageData.version as string;

    log('Getting SVGs...');
    const srcSvgData = await getSvgs(srcSvgDir, distSvgDir, distIconsDir, optimizedOutputSvgDir, optimizeFiles);
...
const cleanDirectories = async (iconDir, distDir, distSvgDir, distIconsDir) => {
  await Promise.all([fs.emptyDir(iconDir), fs.emptyDir(distDir)]);
    await fs.emptyDir(distSvgDir), await fs.emptyDir(distIconsDir);
}

const getSvgs = async (srcDir: string, distSvgDir: string, distIconsDir: string, outputSvgDir: string, isOptimizing: boolean): Promise<SvgData[]> => {
  const optimizedSvgDir = join(distIconsDir, 'svg');
